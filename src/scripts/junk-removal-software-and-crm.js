/**
 * Junk removal industry page interactions.
 * Scoped to #junk-industry-page. Exits if root is absent.
...
(function () {
  ...
  const root = document.getElementById("junk-industry-page");
  if (!root) return;
  ...
  const track = root.querySelector("#junkCapScrollTrack");
  const tabButtons = Array.from(root.querySelectorAll("[data-junk-tab]"));
  ...
  function setActiveTab(index, shouldCenter = true) {
    ...
    if (shouldCenter && isCompact()) {
      ...
      const nav = activeButton.closest("#junkTabsNav");
      if (nav) {
        const scrollLeft =
  ...
  tabButtons.forEach((btn) => {
    btn.addEventListener("click", () => {
      const index = Number(btn.dataset.junkTab || 0);
      setActiveTab(index);
      scrollToStep(index);
    });
  });
