/**
 * HVAC industry page interactions.
 * Scoped to #hvac-industry-page. Exits if root is absent.
...
(function () {
  ...
  const root = document.getElementById("hvac-industry-page");
  if (!root) return;
  ...
  const track = root.querySelector("#hvacCapScrollTrack");
  const tabButtons = Array.from(root.querySelectorAll("[data-hvac-tab]"));
  ...
  function setActiveTab(index, shouldCenter = true) {
    ...
    if (shouldCenter && isCompact()) {
      ...
      const nav = activeButton.closest("#hvacTabsNav");
      if (nav) {
        const scrollLeft =
  ...
  tabButtons.forEach((btn) => {
    btn.addEventListener("click", () => {
      const index = Number(btn.dataset.hvacTab || 0);
      setActiveTab(index);
      scrollToStep(index);
    });
  });
