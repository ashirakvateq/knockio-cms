/**
 * Garage door industry page interactions.
 * Scoped to #gd-industry-page. Exits if root is absent.
...
(function () {
  ...
  const root = document.getElementById("gd-industry-page");
  if (!root) return;
  ...
  const track = root.querySelector("#gdCapScrollTrack");
  const tabButtons = Array.from(root.querySelectorAll("[data-gd-tab]"));
  ...
  function setActiveTab(index, shouldCenter = true) {
    ...
    if (shouldCenter && isCompact()) {
      ...
      const nav = activeButton.closest("#gdTabsNav");
      if (nav) {
        const scrollLeft =
  ...
  tabButtons.forEach((btn) => {
    btn.addEventListener("click", () => {
      const index = Number(btn.dataset.gdTab || 0);
      setActiveTab(index);
      scrollToStep(index);
    });
  });
