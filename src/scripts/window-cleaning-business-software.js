/**
 * Window cleaning industry page interactions.
 * Scoped to #window-industry-page. Exits if root is absent.
...
(function () {
  ...
  const root = document.getElementById("window-industry-page");
  if (!root) return;
  ...
  const track = root.querySelector("#windowCapScrollTrack");
  const tabButtons = Array.from(root.querySelectorAll("[data-window-tab]"));
  ...
  function setActiveTab(index, shouldCenter = true) {
    ...
    if (shouldCenter && isCompact()) {
      ...
      const nav = activeButton.closest("#windowTabsNav");
      if (nav) {
        const scrollLeft =
  ...
  tabButtons.forEach((btn) => {
    btn.addEventListener("click", () => {
      const index = Number(btn.dataset.windowTab || 0);
      setActiveTab(index);
      scrollToStep(index);
    });
  });
