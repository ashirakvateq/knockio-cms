/**
 * Pest control industry page interactions.
 * Scoped to #pc-industry-page. Exits if root is absent.
...
(function () {
  ...
  const root = document.getElementById("pc-industry-page");
  if (!root) return;
  ...
  const track = root.querySelector("#pcCapScrollTrack");
  const tabButtons = Array.from(root.querySelectorAll("[data-pc-tab]"));
  ...
  function setActiveTab(index, shouldCenter = true) {
    ...
    if (shouldCenter && isCompact()) {
      ...
      const nav = activeButton.closest("#pcTabsNav");
      if (nav) {
        const scrollLeft =
  ...
  tabButtons.forEach((btn) => {
    btn.addEventListener("click", () => {
      const index = Number(btn.dataset.pcTab || 0);
      setActiveTab(index);
      scrollToStep(index);
    });
  });
