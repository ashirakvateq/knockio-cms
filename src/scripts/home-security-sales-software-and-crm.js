/**
 * Home security industry page interactions.
 * Scoped to #hs-industry-page. Exits if root is absent.
...
(function () {
  ...
  const root = document.getElementById("hs-industry-page");
  if (!root) return;
  ...
  const track = root.querySelector("#hsCapScrollTrack");
  const tabButtons = Array.from(root.querySelectorAll("[data-hs-tab]"));
  ...
  function setActiveTab(index, shouldCenter = true) {
    ...
    if (shouldCenter && isCompact()) {
      ...
      const nav = activeButton.closest("#hsTabsNav");
      if (nav) {
        const scrollLeft =
  ...
  tabButtons.forEach((btn) => {
    btn.addEventListener("click", () => {
      const index = Number(btn.dataset.hsTab || 0);
      setActiveTab(index);
      scrollToStep(index);
    });
  });
