...
function initCalInline() {
  ...
  const gateDistance = window.innerHeight * 0.5;
  const calendarTop = target.getBoundingClientRect().top + window.scrollY;
  if (calendarTop <= window.innerHeight * 1.5) {
    loadCalendar();
    return;
  }
  ...
  const hasPassedGate = () => window.scrollY >= gateDistance;
  const loadIfAlreadyPassed = () => {
    if (hasPassedGate()) startCalendar();
  };

  scrollHandler = () => {
    if (!hasPassedGate()) return;
    startCalendar();
  };
  window.addEventListener('scroll', scrollHandler, { passive: true });

  loadIfAlreadyPassed();
  window.requestAnimationFrame(loadIfAlreadyPassed);
  window.addEventListener('pageshow', loadIfAlreadyPassed, { once: true });
}
