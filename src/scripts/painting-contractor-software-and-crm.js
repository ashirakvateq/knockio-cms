/**
 * Painting industry page interactions.
 * Scoped to #paint-industry-page. Exits if root is absent.
...
(function () {
  ...
  const root = document.getElementById("paint-industry-page");
  if (!root) return;
  ...
  const track = root.querySelector("#paintCapScrollTrack");
  const tabButtons = Array.from(root.querySelectorAll("[data-paint-tab]"));
  ...
  function setActiveTab(index, shouldCenter = true) {
    ...
    if (shouldCenter && isCompact()) {
      ...
      const nav = activeButton.closest("#paintTabsNav");
      if (nav) {
        const scrollLeft =
  ...
  tabButtons.forEach((btn) => {
    btn.addEventListener("click", () => {
      const index = Number(btn.dataset.paintTab || 0);
      setActiveTab(index);
      scrollToStep(index);
    });
  });
