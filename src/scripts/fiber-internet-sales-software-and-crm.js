...
(function () {
  ...
  const root = document.getElementById("fiber-industry-page");
  if (!root) return;
  ...
  const track = root.querySelector("#fiberCapScrollTrack");
  const tabButtons = Array.from(root.querySelectorAll("[data-fiber-tab]"));
  ...
  function setActiveTab(index, shouldCenter = true) {
    ...
    if (shouldCenter && isCompact()) {
      ...
      const nav = activeButton.closest("#fiberTabsNav");
      if (nav) {
        const scrollLeft =
  ...
  tabButtons.forEach((btn) => {
    btn.addEventListener("click", () => {
      const index = Number(btn.dataset.fiberTab || 0);
      setActiveTab(index);
      scrollToStep(index);
    });
  });
