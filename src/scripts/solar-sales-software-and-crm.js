/**
 * Solar industry page interactions.
 * Scoped to #sl-industry-page. Exits if root is absent.
...
(function () {
  ...
  const root = document.getElementById("sl-industry-page");
  if (!root) return;
  ...
  const track = root.querySelector("#slCapScrollTrack");
  const tabButtons = Array.from(root.querySelectorAll("[data-sl-tab]"));
  ...
  function setActiveTab(index, shouldCenter = true) {
    ...
    if (shouldCenter && isCompact()) {
      ...
      const nav = activeButton.closest("#slTabsNav");
      if (nav) {
        const scrollLeft =
  ...
  tabButtons.forEach((btn) => {
    btn.addEventListener("click", () => {
      const index = Number(btn.dataset.slTab || 0);
      setActiveTab(index);
      scrollToStep(index);
    });
  });
