/**
 * Landscaping industry page interactions.
 * Scoped to #ls-industry-page. Exits if root is absent.
...
(function () {
  ...
  const root = document.getElementById("ls-industry-page");
  if (!root) return;
  ...
  const track = root.querySelector("#lsCapScrollTrack");
  const tabButtons = Array.from(root.querySelectorAll("[data-ls-tab]"));
  ...
  function setActiveTab(index, shouldCenter = true) {
    ...
    if (shouldCenter && isCompact()) {
      ...
      const nav = activeButton.closest("#lsTabsNav");
      if (nav) {
        const scrollLeft =
  ...
  tabButtons.forEach((btn) => {
    btn.addEventListener("click", () => {
      const index = Number(btn.dataset.lsTab || 0);
      setActiveTab(index);
      scrollToStep(index);
    });
  });
