/**
 * Roofing industry page interactions.
 * Scoped to #roof-industry-page. Exits if root is absent.
...
(function () {
  ...
  const root = document.getElementById("roof-industry-page");
  if (!root) return;
  ...
  const track = root.querySelector("#roofCapScrollTrack");
  const tabButtons = Array.from(root.querySelectorAll("[data-roof-tab]"));
  ...
  function setActiveTab(index, shouldCenter = true) {
    ...
    if (shouldCenter && isCompact()) {
      ...
      const nav = activeButton.closest("#roofTabsNav");
      if (nav) {
        const scrollLeft =
  ...
  tabButtons.forEach((btn) => {
    btn.addEventListener("click", () => {
      const index = Number(btn.dataset.roofTab || 0);
      setActiveTab(index);
      scrollToStep(index);
    });
  });
