/**
 * Plumbing industry page interactions.
 * Scoped to #plumb-industry-page. Exits if root is absent.
...
(function () {
  ...
  const root = document.getElementById("plumb-industry-page");
  if (!root) return;
  ...
  const track = root.querySelector("#plumbCapScrollTrack");
  const tabButtons = Array.from(root.querySelectorAll("[data-plumb-tab]"));
  ...
  function setActiveTab(index, shouldCenter = true) {
    ...
    if (shouldCenter && isCompact()) {
      ...
      const nav = activeButton.closest("#plumbTabsNav");
      if (nav) {
        const scrollLeft =
  ...
  tabButtons.forEach((btn) => {
    btn.addEventListener("click", () => {
      const index = Number(btn.dataset.plumbTab || 0);
      setActiveTab(index);
      scrollToStep(index);
    });
  });
