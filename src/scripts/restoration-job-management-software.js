/**
 * Restoration industry page interactions.
 * Scoped to #resto-industry-page. Exits if root is absent.
...
(function () {
  ...
  const root = document.getElementById("resto-industry-page");
  if (!root) return;
  ...
  const track = root.querySelector("#restoCapScrollTrack");
  const tabButtons = Array.from(root.querySelectorAll("[data-resto-tab]"));
  ...
  function setActiveTab(index, shouldCenter = true) {
    ...
    if (shouldCenter && isCompact()) {
      ...
      const nav = activeButton.closest("#restoTabsNav");
      if (nav) {
        const scrollLeft =
  ...
  tabButtons.forEach((btn) => {
    btn.addEventListener("click", () => {
      const index = Number(btn.dataset.restoTab || 0);
      setActiveTab(index);
      scrollToStep(index);
    });
  });
