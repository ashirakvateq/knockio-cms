/**
 * Electrical industry page interactions.
 * Scoped to #elec-industry-page. Exits if root is absent.
...
(function () {
  ...
  const root = document.getElementById("elec-industry-page");
  if (!root) return;
  ...
  const track = root.querySelector("#elecCapScrollTrack");
  const tabButtons = Array.from(root.querySelectorAll("[data-elec-tab]"));
  ...
  function setActiveTab(index, shouldCenter = true) {
    ...
    if (shouldCenter && isCompact()) {
      ...
      const nav = activeButton.closest("#elecTabsNav");
      if (nav) {
        const scrollLeft =
  ...
  tabButtons.forEach((btn) => {
    btn.addEventListener("click", () => {
      const index = Number(btn.dataset.elecTab || 0);
      setActiveTab(index);
      scrollToStep(index);
    });
  });
